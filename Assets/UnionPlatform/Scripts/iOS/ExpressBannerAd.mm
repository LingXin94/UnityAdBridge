...
extern "C" {
    ...
    void UnionPlatform_ExpressBannersAd_Load(
        ...
        CGFloat bannerHeigh = screenWidth/width*height;
        if (0) {
            instance.bannerView = [[BUNativeExpressBannerView alloc] initWithSlotID:[[NSString alloc] initWithUTF8String:slotID] rootViewController:GetAppController().rootViewController adSize:CGSizeMake(screenWidth, bannerHeigh) IsSupportDeepLink:YES interval:30];
        } else {
            instance.bannerView = [[BUNativeExpressBannerView alloc] initWithSlotID:[[NSString alloc] initWithUTF8String:slotID] rootViewController:GetAppController().rootViewController adSize:CGSizeMake(screenWidth, bannerHeigh) IsSupportDeepLink:YES];
        }
        instance.bannerView.frame = CGRectMake(0, CGRectGetHeight([UIScreen mainScreen].bounds)-bannerHeigh, screenWidth, bannerHeigh);
